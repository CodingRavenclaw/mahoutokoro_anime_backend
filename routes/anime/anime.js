  ...
  (req, res) => {
    ...
    MAHOUTOKORO_MASTER_ACCESSOR.getConnection((err, con) => {
      ...
      let stmt = "SELECT anime.anime_id, anime.name AS 'anime_name', studio.name AS 'studio_name', author.first_name AS 'author_first_name', author.last_name AS 'author_last_name', " +
        "anime.year, country.country_id, country.country_flag, anime.is_allowed " +
...
ROUTER.get('/anime/gettotalnumberofanimeindatabase',
  (req, res) => {
    ...
    MAHOUTOKORO_MASTER_ACCESSOR.getConnection((err, con) => {
      if (err) throw err;
      let stmt = "SELECT COUNT(*) AS 'total_number_of_anime_in_database' FROM anime";
      con.query(stmt, (sqlErr, sqlRes, sqlFields) => {
        if (sqlErr) {
          console.error('/anime/gettotalnumberofanimeindatabase: ' + sqlErr);
          arrErrors.push(ERROR_HELPER('SERVER_ERROR'));
          res.status(500).send({'errors': arrErrors});
        } else {
          res.status(200).send({'data': sqlRes});
